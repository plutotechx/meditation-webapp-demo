...
// - checkStatus: full scan (all rows) แก้จาก 125 rows → scan ทุก row
// - getAllStatus: scan แค่ 125 rows ท้ายสุด (วันนี้เท่านั้น ไม่กระทบ)
// - keepAlive_: ping every 5 min via Time-based Trigger (prevents Cold Start)
// - hasDuplicateSubmission_: full scan (all rows) แก้จาก 125 rows → scan ทุก row
...
// - v2.2.1: เพิ่ม cache TTL เป็น 6 ชั่วโมง (21600 วินาที)
// - v2.2.2: buildStatusForFast_ + hasDuplicateSubmission_ scan ทุก row (fix ข้อมูลขาด)
// ===============================
...
// ✅ scan แค่ 125 rows ท้ายสุด (วันนี้เท่านั้น — ไม่แตะ)
function handleGetAllStatus_(p) {
  ...
  // ✅ คงไว้ 125 rows — ใช้แค่วันนี้ ไม่กระทบ index.html
  const SCAN_ROWS = 125;
...
// ====== FAST STATUS SCAN ======
// ✅ v2.2.2: scan ทุก row (แก้จาก 125 rows) เพื่อรองรับข้อมูลย้อนหลัง
...
// ====== DUPLICATE CHECK ======
// ✅ v2.2.2: scan ทุก row (แก้จาก 125 rows) ป้องกัน miss dup เก่า
...
// ====== BUILD DONE MAP (dashboard เท่านั้น) ======
